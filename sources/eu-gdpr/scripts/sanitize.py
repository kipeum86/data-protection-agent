"""EU GDPR sub-KB sanitizer for prompt-injection patterns in untrusted text.

Per AGENTS.md Rule 3: untrusted text from kb/eu-gdpr/library/, MCP outputs,
...
EU-specific patterns can diverge later without cross-sub-KB coupling.
...
def _load_patterns() -> tuple[tuple[tuple[str, str, str], ...], bool]:
    ...
    for module_path in (
        "sources.eu_gdpr.scripts.lib.patterns",
