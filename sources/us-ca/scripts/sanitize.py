"""CA sub-KB sanitizer for prompt-injection patterns in untrusted text.

Per AGENTS.md Rule 3: untrusted text from raw/, library/, or external
fetches must be sanitized before entering an LLM context window.
...
def _load_patterns() -> tuple[tuple[tuple[str, str, str], ...], bool]:
    ...
    for module_path in (
        "sources.us_ca.scripts.lib.patterns",
