...
- **`GDPR-expert`** *(별도 레포)* → EU GDPR + ePrivacy Directive + EU AI Act + Data Act + Data Governance Act · 1,029개 레코드
- **`PIPA-expert`** *(별도 레포)* → 개인정보 보호법 (PIPA) + 정보통신망법 + 신용정보법 + 위치정보법 + PIPC 가이드라인 · 929개 레코드
- **California 서브-KB** *(이 레포 내 `sources/us-ca/`)* → CCPA-as-amended-by-CPRA + CPPA 규정 + CIPA + CMIA + AADC + Customer Records Act + 인접 프라이버시 법령 · 237개 레코드

> **이 레포만 봐도 충분합니다.** 별도 sibling 레포를 일일이 열어볼 필요 없도록, README 와 docs 가 자체 완결적으로 작성되어 있습니다. 단, EU/KR 지식베이스의 source-of-truth 는 여전히 sibling 레포가 보유하며 (별도로 유지·릴리스됨), 결정론적 빌드 스크립트로 이 레포에 다시 import 됩니다. California 서브-KB 는 이 레포에서 직접 빌드·관리됩니다.
...
|:---|:---|---:|:---|:---|
| `eu-gdpr` | sibling [`GDPR-expert/`](https://github.com/kipeum86/GDPR-expert) | **1,029** | GDPR | Articles · Recitals · EDPB 문서 (guidelines/opinions/binding decisions) · CJEU 판례 · enforcement decisions |
| `kr-pipa` | sibling [`PIPA-expert/`](https://github.com/kipeum86/PIPA-expert) | **929** | 개인정보 보호법 | 법조문 · 시행령 조문 · 정보통신망법 · 신용정보법 · 위치정보법 · PIPC 가이드라인 · 법원 판결 |
| `us-ca` | local `sources/us-ca/` | **237** | CCPA-as-amended-by-CPRA | CCPA 본법 · CPPA 규정 (11 CCR § 7000–7300) · CalOPPA · CIPA · CMIA · AADC · Customer Records Act · OAG 가이드 · 법원 판결 |
| **합계** | | **2,195** | | |
...
```mermaid
flowchart LR
    subgraph siblings["Source of Truth (별도 유지)"]
        direction TB
        G["GDPR-expert/<br/><i>EU 입법은 CELLAR API,<br/>EDPB PDF, CJEU 판례 파일</i>"]
        P["PIPA-expert/<br/><i>law.go.kr 구조화 데이터,<br/>PIPC 가이드라인 PDF</i>"]
        C["sources/us-ca/<br/><i>CPPA 규정, OAG 가이드,<br/>SCOCAL 미러, leginfo</i>"]
    end

    IM["<b>scripts/import_namespaced_kbs.py</b><br/>(결정론적, idempotent)"]

    subgraph runtime["Runtime KB (이 레포)"]
        direction TB
...
EU/KR 의 source-of-truth 는 sibling 레포가 보유합니다 (별도 유지·릴리스 — 인제스트 파이프라인은 [GDPR-expert README](https://github.com/kipeum86/GDPR-expert), [PIPA-expert README](https://github.com/kipeum86/PIPA-expert) 참조). importer 는 그쪽의 구조화된 `library/` + `index/` 트리를 `kb/{namespace}/` 로 복사합니다. California 는 모든 것이 `sources/us-ca/` 에 있습니다: per-section 마크다운은 `library/grade-{a,b,c}/`, 패밀리별 JSON 인덱스는 `index/`, 빌드 파이프라인은 `scripts/build_california_kb.py`. importer 는 이를 `kb/us-ca/` 로 복사합니다.
...
│   ├── eu-gdpr/               #   ← sibling GDPR-expert 에서 import
│   ├── kr-pipa/               #   ← sibling PIPA-expert 에서 import
│   └── us-ca/                 #   ← 로컬 sources/us-ca/ 에서 import
...
| ⏳ 검토 중 | KR 판례 import | 별도 sibling 레포 필요 (GDPR-expert / PIPA-expert 와 평행) |
